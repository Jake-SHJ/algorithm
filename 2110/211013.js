...
// 순서대로 빼내야 하는 샌드위치를 queue로 되어있는 학생 순서대로 가져갈 때, 못 먹는 학생의 수를 반환하는 문제
...
var countStudents = function (students, sandwiches) {
  ...
  while (students.length > 0) {
    if (students[0] === sandwiches[0]) {
      ...
      sandwiches.shift();
      counter = 0;
      // 같으면 둘 다 제거하고, 카운터 초기화
    } else {
      const temp = students[0];
      students.shift();
      students.push(temp);
      counter++;
      // 다르면 학생은 뒤로 보내고, 카운터 증가
    }

    // 계속 카운터가 쌓여서 샌드위치 수만큼 돌면 반복문 탈출
    if (counter === sandwiches.length) break;
  }

  // 카운터 반환 (다 먹었으면 카운터 초기화로 0이 반환, 아니면 못먹은 학생 수 === 못먹은 샌드위치 수 반환)
  return counter;
};
