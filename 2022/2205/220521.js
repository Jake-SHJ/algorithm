...
// 주어진 조건에 맞춰 지불 금액을 반환하는 로직을 완성하는 문제
...
var Cashier = function (n, discount, products, prices) {
  this.priceTag = new Map();
  for (let i = 0; i < products.length; i++) {
    this.priceTag.set(products[i], prices[i]);
  }
  this.count = 0;
  this.target = n;
  this.dc = discount;
...
Cashier.prototype.getBill = function (product, amount) {
  ...
  for (let i = 0; i < product.length; i++) {
    total += amount[i] * this.priceTag.get(product[i]);
  }

  if (this.count % this.target === 0) return total * (1 - this.dc / 100);
